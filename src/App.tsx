...
export default function App() {
  ...
  const [data, setData] = useState<BirthdayData>(() => {
    const saved = localStorage.getItem('birthday_tribute_data_v6');
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        if (parsed.name === "Mariana") {
          localStorage.removeItem('birthday_tribute_data_v6');
  ...
  const handleUpdateData = (newData: BirthdayData) => {
    setData(newData);
    localStorage.setItem('birthday_tribute_data_v6', JSON.stringify(newData));
  };
  ...
  const handleResetData = () => {
    if (window.confirm("Deseja realmente voltar aos dados padrões de exemplo para a Lunna? Isso limpará suas edições personalizadas atuais.")) {
      ...
      localStorage.setItem('birthday_tribute_data_v6', JSON.stringify(defaultBirthdayData));
    }
  };
